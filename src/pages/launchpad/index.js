...
const AllLaunchpadComponent = () => {
  return (
    <section className="sm:p-6 p-2 bg-black shadow flex-col justify-center gap-8 flex">
      <div className="flex sm:justify-between justify-center items-center sm:gap-[100px]">
        <div className="sm:w-[530px] h-12 bg-neutral-950 items-center inline-flex">
        ...
        <div className="h-6 gap-9 sm:inline-flex hidden text-white text-xs uppercase leading-3 text-nowrap">
      ...
      <div className="grid sm:grid-cols-3 gap-10 font-['Sora']">
        {allLaunchpadData.map((item, index) => {
          return (
            <CoinItem key={index} data={item} />
          )
        })}
      </div>
    </section>
...
const CoinItem = ({ data }) => {
  ...
  return (
    <div className="relative flex flex-col gap-6 px-4 pt-4 pb-5 border border-[#26242C]">
      {data.affiliate === true && (
        <div className="absolute left-0 right-0 -top-5 mx-auto w-fit text-xs font-bold leading-5 text-black px-3 py-2 rounded-full border-[2px] border-[#4A3A20] bg-[#F6C36A]">Affiliate {data.affiliate_percent}%</div>
      )}
      <div className={clsx("absolute right-4 top-4 flex p-2 items-center gap-[6px] border", data.status === "upcoming" ? "border-[#F6C36A] text-[#F6C36A]" : data.status === "sale live" ? "border-[#7ADB78] text-[#7ADB78]" : "border-[#7F7F7F] text-[#7F7F7F]")}>
        <svg xmlns="http://www.w3.org/2000/svg" width="6" height="6" viewBox="0 0 6 6" fill="none">
          <circle cx="3" cy="3" r="3" fill="currentColor" />
        </svg>
        <p className="text-xs leading-5">{data.status === "upcoming" ? "Upcoming" : data.status === "sale live" ? "Sale Live" : "Canceled"}</p>
      </div>
      ...
      <div className="flex justify-between items-center border-t-[1px] border-[#26242C] pt-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <button type="button" className="text-xs font-bold font-['Source+Code+Pro'] leading-5 uppercase text-[#1A1A1A] bg-[#FE4F4C] px-4 py-3" onClick={() => navigate("/detail")}>view</button>
        </div>
      </div>
    </div>
  )
...
const LaunchpadPage = () => {
  ...
  return (
    <section className="flex flex-col gap-6 py-[72px] sm:max-w-7xl mx-auto">
      <div className="flex flex-col gap-[61px]">
        <p className="sm:text-[32px] text-[28px] sm:leading-5 text-[#FFF] uppercase">
          Current Presales
        </p>
        <div className="flex w-full text-center">
          <div className={clsx("flex justify-center items-center text-sm uppercase sm:p-4 p-1 border border-[#FFF] w-1/3 cursor-pointer", currentTab === "ALL LAUNCHPADS" ? "text-[#1A1A1A] bg-[#FFF]" : "text-[#999] bg-[#000]")} onClick={() => setCurrentTab("ALL LAUNCHPADS")}>
            All launchpads
          </div>
          <div className={clsx("flex justify-center items-center text-sm uppercase sm:p-4 p-1 border border-[#FFF] w-1/3 cursor-pointer", currentTab === "ADVANCED MODE" ? "text-[#1A1A1A] bg-[#FFF]" : "text-[#999] bg-[#000]")} onClick={() => setCurrentTab("ADVANCED MODE")}>
            Advanced Mode
          </div>
          <div className={clsx("flex justify-center items-center text-sm uppercase sm:p-4 p-1 border border-[#FFF] w-1/3 cursor-pointer", currentTab === "MY CONTRIBUTIONS" ? "text-[#1A1A1A] bg-[#FFF]" : "text-[#999] bg-[#000]")} onClick={() => setCurrentTab("MY CONTRIBUTIONS")}>
            My Contributions
          </div>
        </div>
      </div>
